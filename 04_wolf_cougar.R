...
mod_spec <- nimbleCode({
  ...
  for (t in 2:length_t) {
    mu_elk[t] <- N_elk[t - 1] + r_elk * N_elk[t - 1] * (1 - 
      ...
      beta_elk * N_wolf[t - 1] * N_elk[t - 1] -
      gamma_elk * N_cougar[t - 1] * N_elk[t - 1]
    
    
    mu_bison[t] <- N_bison[t - 1] + r_bison * N_bison[t - 1] * (1 - 
                                                                  alpha_bb * N_bison[t - 1] - 
                                                                  alpha_be * N_elk[t - 1]) -
      beta_bison * N_wolf[t - 1] * N_bison[t - 1]
...
# beta_elk
plot(1:nrow(samples), samples[,"beta_elk"], type = "l")

# beta_bison
plot(1:nrow(samples), samples[,"beta_bison"], type = "l")

# gamma_elk
plot(1:nrow(samples), samples[,"gamma_elk"], type = "l")
...
# Save ----
write.csv(samples, "out/MCMC_samples_wolf_cougar.csv", row.names = FALSE)
write.csv(samples2, "out/MCMC_samples2_wolf_cougar.csv", row.names = FALSE)
